<%-- 
    Document   : footer
    Created on : 20/04/2015, 03:53:14 PM
    Author     : VíctorAndrés
--%>


</div>
</div>

</div>
</div>

</div>
<!-- END PAGE CONTENT WRAPPER -->                
</div>            
<!-- END PAGE CONTENT -->
</div>
<!-- END PAGE CONTAINER -->

<!-- MESSAGE BOX-->
<div class="message-box animated fadeIn" data-sound="alert" id="mb-signout">
    <div class="mb-container">
        <div class="mb-middle">
            <div class="mb-title"><span class="fa fa-sign-out"></span> Log <strong>Out</strong> ?</div>
            <div class="mb-content">
                <p>Esta Seguro que desea salir del sistema?</p>                    
                <p>Presione NO si quiere seguir trabajando. Presione SI para cerrar la sección.</p>
            </div>
            <div class="mb-footer">
                <div class="pull-right">
                    <a href="/agricultura-online/index.jsp" class="btn btn-success btn-lg">Yes</a>
                    <button class="btn btn-default btn-lg mb-control-close">No</button>
                </div>
            </div>
        </div>
    </div>
</div>
<!-- MESSAGE BOX-->


<!-- START PRELOADS -->
<audio id="audio-alert" src="/agricultura-online/audio/alert.mp3" preload="auto"></audio>
<audio id="audio-fail" src="/agricultura-online/audio/fail.mp3" preload="auto"></audio>
<!-- END PRELOADS -->                 

<!-- START SCRIPTS -->
<!-- START PLUGINS -->
<script type="text/javascript" src="/agricultura-online/js/plugins/jquery/jquery.min.js"></script>
<script type="text/javascript" src="/agricultura-online/js/plugins/jquery/jquery-ui.min.js"></script>
<script type="text/javascript" src="/agricultura-online/js/plugins/bootstrap/bootstrap.min.js"></script>        
<!-- END PLUGINS -->

<!-- THIS PAGE PLUGINS -->
<script type='text/javascript' src='/agricultura-online/js/plugins/icheck/icheck.min.js'></script>
<script type="text/javascript" src="/agricultura-online/js/plugins/mcustomscrollbar/jquery.mCustomScrollbar.min.js"></script>
<script type="text/javascript" src="/agricultura-online/js/plugins/datatables/jquery.dataTables.min.js"></script> 
<script type="text/javascript" src="/agricultura-online/js/plugins/bootstrap/bootstrap-datepicker.js"></script>
<script type="text/javascript" src="/agricultura-online/js/plugins/bootstrap/bootstrap-timepicker.min.js"></script>
<script type="text/javascript" src="/agricultura-online/js/plugins/bootstrap/bootstrap-colorpicker.js"></script>
<script type="text/javascript" src="/agricultura-online/js/plugins/bootstrap/bootstrap-file-input.js"></script>
<script type="text/javascript" src="/agricultura-online/js/plugins/bootstrap/bootstrap-select.js"></script>
<script type="text/javascript" src="/agricultura-online/js/plugins/tagsinput/jquery.tagsinput.min.js"></script>
        <script type="text/javascript" src="/agricultura-online/js/plugins/morris/raphael-min.js"></script>
        <script type="text/javascript" src="/agricultura-online/js/plugins/morris/morris.min.js"></script>
<!-- END PAGE PLUGINS -->         

<!-- START TEMPLATE -->
<script type="text/javascript" src="/agricultura-online/js/settings.js"></script>
<script type="text/javascript" src="/agricultura-online/js/plugins.js"></script>        
<script type="text/javascript" src="/agricultura-online/js/actions.js"></script> 
<script type="text/javascript" src="/agricultura-online/js/demo_charts_morris.js"></script>
<!-- END TEMPLATE -->
<!-- END SCRIPTS -->         
</body>
</html>
